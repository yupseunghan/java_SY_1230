...
public class StudentMain {
	...
	private static void deleteSubject() {
		...
		String index = subjectList.indexOf(setSub);
		if(index != -1) {
	...
	private static void updateStudent() {
		...
		Student updateStudent = new Student(grade, classNum, num, name);
		//수정 할 자리에 객체가 있으면 리턴
	...
	private static void insertStudent() {
		...
		Student setStd = new Student(grade,classNum,num,name);
		setStd.list= new ArrayList<SubjectScore>();
		//생성한 객체가 리스트에 있는지 확인하여 있으면 종료 => Student클래스의 equals를 오버라이딩
		//리스트.indexOf(객체2) => Objects.equals(객체1, 객체2) => 객체1.equals(객체2)
		if(studentList.indexOf(setStd) != -1) {
	...
	private static Subject setSubject() {
		...
		removeBuffer();
		String name = sc.nextLine();
		Subject setSub = new Subject(grade,semester,name);
		return setSub;
	}
	private static Student setStudent() {
		...
		int num = sc.nextInt();
		//입력받은 정보로 객체 생성
		Student setStd = new Student(grade,classNum,num,"");
		return setStd;
	}
}